...
class Route
{
    ...
    public static function init()
    {
        self::parseUrlPath();
        @self::ruleInit();
        self::$module = (isset(self::$pathInfo[0]) && self::$pathInfo[0] != '') ? self::$pathInfo[0] : Config::get('default_module');
    ...
    {
        if (!is_dir(BX_ROOT . "app/" . self::$module)) {
            show_error('module ' . self::$module . ' is not exists!');
        }
        $controller = 'app\\' . self::$module . '\controller\\' . self::$controller;
        $action = self::$action;
