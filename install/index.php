...
<head>
    ...
    if (is_file('install.lock')) {
        exit('您已经安装了 如果要重新安装请删除install.lock文件');
    }
    ...
    if ($do == 'yes') {
        ...
        if ($host == null || $port == null || $user == null || $name == null || $pre == null || $key == null) {
            exit('除了数据库密码其他请进行全部填写');
        } else {
            $config = "<?php
return [
...
?>";
        ...
        try {
            $con = new PDO("mysql:host={$host};port={$port};dbname={$name}", $user, $pw);
            $con->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $con->exec("set names utf8");
            $sql = file_get_contents("install.sql");
            $sql = str_replace('{ABC}', $pre, $sql);
            $sql = explode(';</explode>', $sql);
            for ($i = 0; $i < count($sql) - 1; $i++) {
                $con->exec($sql[$i]);
            }
            echo '共执行' . (count($sql) - 1) . '条sql语句<br />';
            if (file_put_contents('install.lock', 'lock')) {
                echo '写入安装锁文件成功' . '<br />';
            } else {
                exit('写入安装锁文件失败');
            }
            if (file_put_contents('update.lock', 'lock')) {
                echo '写入升级锁文件成功' . '<br />';
            } else {
                exit('写入升级锁文件失败');
            }
            ...
            exit('恭喜！安装成功 默认账号 admin 默认密码 admin');
        } catch (PDOException $e) {
            exit('<h3>安装出错:' . $e->getMessage() . '</h3>');
        }
    } ?>
