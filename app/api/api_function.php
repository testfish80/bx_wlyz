...
function bx_encrypt($data, $key, $type)
{
    switch ($type) {
        case 'authcode':
            return authcode($data, '', $key);
            break;
        case 'rc4':
            return base64_encode(rc4($key, $data));
            break;
        case 'no':
            return $data;
            break;
        case 'defined_encrypt':
            return @call_user_func_array('bx_defined_encrypt', array($data, $key));
            break;
        default:
            break;
    }
...
function bx_decrypt($data, $key, $type)
{
    switch ($type) {
        case 'authcode':
            return authcode($data, 'DECODE', $key);
            break;
        case 'rc4':
            return rc4($key, base64_decode($data));
            break;
        case 'no':
            return stripslashes($data);
            break;
        case 'defined_encrypt':
            return @call_user_func_array('bx_defined_decrypt', array($data, $key));
            break;
        default:
            break;
    }
